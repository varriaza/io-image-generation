...
class Lightning(BaseModel):
    ...
    def run_model(self) -> None:
        ...
        # Load model
        unet = UNet2DConditionModel.from_config(base, subfolder="unet").to("cuda", torch_dtype)
        unet.load_state_dict(load_file(hf_hub_download(repo, ckpt), device="cuda"))

        pipe = StableDiffusionXLPipeline.from_pretrained(base, unet=unet, torch_dtype=torch_dtype, variant=variant).to("cuda")
